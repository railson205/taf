<button onclick="exportarPDF('<?= $nome_tabela ?>')" class="btn btn-danger">
    Exportar PDF
</button>
<table class="table table-striped table-hover table-bordered" style="width: 100%;"
    data-export-title="Tabela de Índices e Notas - <?= $nome_tabela ?> " id="tabelaIndices<?= $nome_tabela ?>">
    ...
    <tbody>
        <?php if (!empty($infoNotas[(string) $nome_tabela]) && is_array($infoNotas[(string) $nome_tabela])):
            ...
            foreach ($infoNotas[(string) $nome_tabela] as $n) {
                $agrupado[] = $n['nome_do_exercicio'] . '|' . $n['indice'] . '|' . $n['modo_de_contagem'] . '----' . $n['faixa_etaria'] . '|' . $n['valor_nota'];
            }
            $agrupado = ordena_array_indice($agrupado);
            $linhas = [];
            foreach ($agrupado as $grupo) {
                //Separa o conjunto de faixa etária + nota e nome_do_exercicio + indice
                $tupla = explode('----', $grupo);
                $verificacao = 0;
...
<script>
    function exportarPDF(nomeTabela) {

    // Remove classe anterior
    document.querySelectorAll('.print-area')
        .forEach(el => el.classList.remove('print-area'));

    const tabela = document.getElementById("tabelaIndices" + nomeTabela);

    if (!tabela) return;

    tabela.classList.add("print-area");

    window.print();

    // Remove depois da impressão
    tabela.classList.remove("print-area");
}
